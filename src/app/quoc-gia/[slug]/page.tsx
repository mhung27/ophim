...
export default async function CountryPage({ params, searchParams }: PageProps) {
    ...
    let data;
    try {
        data = await fetchMoviesByCountry(slug, currentPage);
    } catch (error) {
        console.error(error);
        notFound();
    }
    ...
    return (
        <div className="flex flex-col min-h-screen">
            <div className="flex-grow container mx-auto px-4 py-8 space-y-8 min-h-[60vh]">
                ...
                {movies.length === 0 ? (
                    <div className="text-white/50 text-lg text-center py-20">Hiện tại chưa có phim nào thuộc quốc gia này.</div>
                ) : (
